...
返回的 `BondZhCovRecord` 包含 72 个字段，完整说明见 [返回字段文档](docs/interfaces/BondZhCovRecord.md)。新增字段包括 `resaleTriggerPrice`（回售触发价）、`redeemTriggerPrice`（强赎触发价）、`pbRatio`（正股市净率）、`couponRate`（本期票息）、`convertStartDate`、赎回/回售条款及执行信息。
